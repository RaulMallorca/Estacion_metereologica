...
//============================== Pines usados
const int Anemometro = D5;  //Definimos el pin de interrupción para el anemómetro
#define direccion_Viento A0 // pin conectado a Vout mediante la una resistencia de 10kOhm. Vout-resistencia-pin
...
int valor_Grados_Pin_Veleta = 0;
int valor_Cardinal_Pin_Veleta = 0;
float angulo = 0;     
//char* punto_Cardinal = "temp";     
...
/*------------------------FUNCIÓN PARA HAYAR PUNTOS CARDINALES VELETA------------------------*/
/*void p_Cardinales (int valor_Cardinal_Pin_Veleta) 
{
  //============================== Arrays específicos para la veleta con una resistencia de 10kOhm conectada al Vout del NodeMCU.
int sensorExp [] = {74, 92, 102, 134, 198, 259, 304, 427, 482, 620, 652, 725, 808, 848, 906, 963};
char* dirCardinal[] = {"ESE","ENE","E","SSE","SE","SSW","S","NNE","NE","WSW","SW","NNW","N","WNW","NW","W"};

int sensorMin[] = {73, 90, 100, 132, 197, 258, 303, 424, 480, 618, 649, 723, 804, 845, 903, 960};
int sensorMax[] = {77, 95, 105, 137, 201, 262, 307, 430, 485, 624, 655, 728, 812, 852, 910, 966};

valor_Cardinal_Pin_Veleta = analogRead(direccion_Viento);
  for(int i=0; i<=15; i++) {
   if(valor_Cardinal_Pin_Veleta >= sensorMin[i] && valor_Cardinal_Pin_Veleta <= sensorMax[i]) {
    punto_Cardinal = dirCardinal[i]; 
    break;
   } 
  }
}*/
...
/*--------------FUNCIÓN PARA MEDIR LA VELOCIDAD DEL VIENTO CON EL ANEMÓMETRO--------------*/
 void medicion_anemometro() 
 {
  ...
  Velocidad_Viento = (float)InterruptCounter / (float)RecordTime * 2.4; //Pasamos a km/h. Cada vuelta del anemómetro equivale a
                                                                        //1,492 mph.
}
/*----------------------------------FIN FUNCIÓN----------------------------------*/

/*-----------------------FUNCIÓNFUNCIÓN DE INTERRUPCIÓN ANEMÓMETRO---------------------*/
ICACHE_RAM_ATTR void countup() 
{
  InterruptCounter++; //sumamos uno al contador de interrupción
}
/*----------------------------------FIN FUNCIÓN----------------------------------*/
...
void setup()
{
  ...
  //p_Cardinales (valor_Cardinal_Pin_Veleta); //Si se añade al thing, bloquea el sketch
