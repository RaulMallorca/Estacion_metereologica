...
void timer () {
  ...
  if ((millis() - ultimaConsulta) > tiempoConsulta) {
    ...
    /*--------VARIABLES QUE FALTAN POR AÑADIR CUANDO LLEGUEN LOS SENSORES------------*/
    /* dViento = varExtract ("Dviento", lastBucketValue);
       fViento = varExtract ("Fviento", lastBucketValue);
       
       lLuvia = varExtract ("Lluvia", lastBucketValue);*/
    ...
    /*------Comentar una vez comprobado que interpreta bien los datos------*/
    /* Serial.print ("la cantidad de lluvia es: ");
       Serial.print (lLuvia);
       Serial.println ("mm2"); */
    /*----------------------------FIN COMENTAR----------------------------*/
...
void httprequest_DATA_BASE () {
  ...
  url += altitud;
  url += "&dViento=";
  url += 0; //Sustituir cuando esté el sensor por dViento
  url += "&fViento=";
  url += 1; //Sustituir cuando esté el sensor por fViento
  url += "&uV=";
  url += uV; 
  url += "&lLuvia=";
  url += 3; //Sustituir cuando esté el sensor por lLuvia
  url += "&lumi=";
  url += lumi;
